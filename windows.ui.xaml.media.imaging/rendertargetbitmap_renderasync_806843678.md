...
If you need a pixel buffer representation of the rendered content and not just an image source, use [GetPixelsAsync](rendertargetbitmap_getpixelsasync.md) after calling [RenderAsync](rendertargetbitmap_renderasync.md).
...
Calling [RenderAsync](rendertargetbitmap_renderasync.md) will fail if the scaled height or width of the [UIElement](../windows.ui.xaml/uielement.md) visual tree are larger than the amount of video memory the system is able to allocate internally.
